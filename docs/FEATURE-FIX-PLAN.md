...
| A2 | Add inline Approve button to investor list | src/admin/* | Show Approve/Reject buttons directly in investor table rows for PENDING investors, not just in detail view |
...
| A5 | Wire unlock account button | src/admin/*, src/api.js | Add `unlockInvestor(id)` to api.js. Add "Unlock" button to investor detail when `lockedUntil` is set. Backend endpoint already exists at admin.js:431 |
| A6 | Add empty dashboard state | src/pages/ (was App.jsx):486-509 | When `myProjects.length === 0`, show "Your portfolio is being set up. You'll see your investments here once assigned." instead of all zeros |
| A7 | Fix IRR NaN display | src/pages/ (was App.jsx):497 | Add null/NaN check, show "—" instead of "NaN%" |
...
| # | Task | Files | Detail |
|---|------|-------|--------|
| B1 | ~~Fix download to use secure endpoint~~ | src/pages/Documents.jsx | ✅ DONE — Row click opens preview, action button triggers download via secure endpoint |
| B2 | ~~Fix document assignment to preserve tracking~~ | server/routes/admin.js | ✅ DONE — Upsert logic preserves viewedAt/downloadedAt. Also added group assignment (groupIds param) |
| B3 | ~~Add PDF preview to sign modal~~ | src/pages/Documents.jsx | ✅ DONE — Preview modal with iframe, "View Document" in sign modal opens preview |
| B4 | ~~Add signer status table to admin doc detail~~ | src/admin/DocumentManager.jsx, server/routes/admin.js | ✅ DONE — signatureRequests included in detail endpoint, signer status UI with reminder button |
| B5 | Add unmatched K-1 reassignment | src/admin/DocumentManager.jsx, server/routes/documents.js | Already built — bulk K-1 upload shows unmatched files with manual assign dropdown |
| B6 | ~~Add signature reminder/resend~~ | src/admin/DocumentManager.jsx | ✅ DONE — "Send Reminder" button on pending signers in signer status section |

**Status:** ✅ Sprint B COMPLETE (2026-03-19)
**Commits:** `9c431e1` (e-sign fixes), `cfe3100` (group assign, preview, acknowledge, sig display)
...
| C3 | Add message search | src/admin/*, src/App.jsx | Add search input above message list, filter threads by subject/body containing search term |
| C4 | Add message sort | src/admin/*, src/App.jsx | Sort dropdown: newest first (default), oldest first, unread first |
...
| # | Task | Files | Detail |
|---|------|-------|--------|
| D1 | Add cap table CRUD | src/admin/*, server/routes/admin.js | Add "Add Entry" form (holder name, class, committed, called, ownership %). Add edit/delete buttons per row. Backend: POST/PUT/DELETE `/admin/projects/:id/cap-table` |
| D2 | Add waterfall tier editing | src/admin/*, server/routes/admin.js | Make tier rows editable (name, lpShare, gpShare, threshold). Add "Add Tier" / "Delete Tier" buttons. Backend: PUT/DELETE `/admin/projects/:id/waterfall-tiers/:tierId` |
| D3 | Add "Assign Investor" from project detail | src/admin/* | Add "Add Investor" button on project detail investors tab that opens investor picker + commitment form |
| D4 | Add photo upload on project updates | src/admin/*, server/routes/admin.js:205 | Add file input to update form. Store photo via storage adapter. Add `photoUrl` field to ProjectUpdate model |
| D5 | Add bulk distribution recording | src/admin/*, server/routes/finance.js | "Record Distribution" button on project detail that lets admin enter total distribution amount → auto-splits pro-rata across all investors based on ownership % → creates CashFlow records AND updates each InvestorProject's called/currentValue fields. Currently recording a cash flow (finance.js:79) does NOT update InvestorProject numbers |
| D6 | Add bulk capital call recording | src/admin/*, server/routes/finance.js | Same pattern as D5 but for capital calls — enter amount → split across investors → update InvestorProject.called → create CashFlow records |
| D7 | Add project financial summary | src/admin/* | Show totals on project detail: total committed, total called, total distributed, unfunded commitment. Currently only shows per-investor but no project-level aggregation |
...
| # | Task | Files | Detail |
|---|------|-------|--------|
| E1 | Dashboard project cards drill down | src/pages/ (was App.jsx):524 | Change `onClick={() => onNavigate("portfolio")}` to `onClick={() => { setSelectedProject(p); onNavigate("portfolio"); }}` so it opens the specific project |
| E2 | Add tab interface to investment detail | src/pages/ (was App.jsx):652-812 | Replace vertical stack with tabs: Overview, Documents, Updates, Distributions. Keep capital account in Overview tab |
...
| E4 | Make dashboard recent messages clickable | src/pages/ (was App.jsx):612-632 | Clicking a message should navigate to messages tab and open that thread |
| E5 | Add distribution detail view | src/pages/ (was App.jsx):1310-1323 | Click distribution row to see: waterfall breakdown, payment method, tax withholding |
...
| F2 | Wire notification bell to activity page | src/pages/ (was App.jsx):3003-3009 | Bell click opens activity page instead of messages |
...
| G3 | Add prospect → investor conversion | server/routes/admin.js, src/admin/* | "Convert to Investor" button on prospect detail that creates user account + sends welcome email |
| G4 | Add investor permission customization | prisma/schema.prisma, server/routes/admin.js, src/admin/* | Add `permissions` JSON field to User model. Admin UI to toggle: can_view_cap_table, can_view_waterfall, can_view_documents, can_download, can_message. Backend middleware checks these per-endpoint |
...
| # | Task | Files | Detail |
|---|------|-------|--------|
| H1 | Add copy-to-clipboard for MFA backup codes | src/pages/ (was App.jsx):1926-1930 | "Copy All" button using navigator.clipboard.writeText |
| H2 | Fix password validation mismatch | src/pages/ (was App.jsx):1817-1821 | Add special character requirement to match strength bar |
| H3 | Add login history pagination | src/pages/ (was App.jsx):1954 | "Load More" button or pagination instead of hardcoded 10 |
| H4 | Add profile fields | src/pages/ (was App.jsx):1971-2077 | Add phone number, mailing address inputs. Backend already accepts these if schema is updated |
...
| I3 | Add demo toast notification | src/admin/* | When a demo-mode write returns, show toast: "Demo mode — changes not saved" so users know it's a demo |
...
| J6 | Add remove-investor-from-project endpoint | server/routes/admin.js, src/admin/*, src/api.js | DELETE `/admin/investors/:userId/projects/:projectId`. Add "Remove" button on project investor list |
...
| # | Task | Files | Detail |
|---|------|-------|--------|
| L1 | Add CSV export of investor list | src/admin/*, server/routes/admin.js | "Export CSV" button. Include: name, email, status, group, total committed, total called |
| L2 | Add group filter to investor list | src/admin/* | Dropdown filter to show investors in a specific group |
| L3 | Add group-based message targeting | src/admin/* | In compose message, add "Send to Group" option that selects all investors in chosen group |
...
| L6 | Add investor activity timeline | src/admin/*, server/routes/admin.js | Timeline view in investor detail: logins, downloads, messages, distributions |
